...
try {
  ...
  // 28. Package smoke suite is configured for 31 checks in v1.4.3.
  ...
  console.log(`Provider real handshake preflight smoke passed: 2 candidates · 31 checks suite · no real key read · no real network · fallback=localdev_mock`);
} finally {
  await handle.close();
}
