...
- Added `scripts/localdev-qwen-service-template.mjs`, `npm run service:localdev:qwen-template`, and `npm run adapter:localdev:qwen-websocket-template` as a reusable two-terminal template for replacing fake handlers with real local Qwen2.5-Omni realtime inference.
- Added `npm run test:localdev-adapter-contract:qwen-template` to automatically start the reusable template service and verify the adapter contract against it.
- Added `scripts/localdev-health-check.mjs` and `npm run health:localdev*` commands as no-side-effect WebSocket probes for already-running adapter/template services.
- Documented the runtime/process boundary: `test:localdev-*` scripts may spawn temporary child processes and must not be called from Web UI or Runtime code; normal development should use long-lived service processes.
- Added the generic `websocket_json` Qwen realtime transport slot plus `scripts/localdev-qwen-realtime-transport-smoke.mjs` / `npm run test:localdev-qwen-transport` to verify session.start, audio_frame, camera_frame, input_packet, interrupt, session.close, inbound structured output_turn, and native reply_audio_frame messages against a fake local WebSocket service.
- Added `scripts/localdev-adapter-contract-smoke.mjs`, `npm run test:localdev-adapter-contract`, `npm run test:localdev-adapter-contract:qwen-loopback`, and `npm run test:localdev-adapter-contract:qwen-websocket` as one-shot LocalDev adapter protocol smoke tests. The default scenario validates placeholder reply audio; the Qwen loopback and WebSocket scenarios validate the realtime session boundary, and the WebSocket scenario now normalizes a fake service `omni.output_turn.v1` plus forwards native `omni.reply_audio_frame.v1` without generating speech from text.
- Added early native reply-audio forwarding from providers that expose `onReplyAudioFrame(listener)`, so the adapter skeleton can forward `omni.reply_audio_frame.v1` before the structured `omni.output_turn.v1` arrives.
- Strengthened the Qwen WebSocket smoke tests with ordering assertions: native reply audio must arrive before the structured output turn envelope, protecting the realtime-call path from regressing into text-turn-first playback.
