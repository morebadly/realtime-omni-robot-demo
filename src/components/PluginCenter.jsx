...
export default function PluginCenter({ plugins, onToggle, onRun, onAdd, onDelete }) {
  ...
  return (
    <section className="panel plugin-panel">
      ...
      <p className="muted">空调、邮件、表情、角色切换和实体动作都不是独立用户入口，而是插件动作库。v0.8 为每个插件生成 manifest，权限声明先过 Runtime Guard，再进入 Tool Engine。</p>

      <div className="tool-action-library">
        <div>
          <strong>插件动作库</strong>
      ...
      <form className="plugin-builder orchestration-builder" onSubmit={submitPlugin}>
      ...
      <form className="code-plugin-builder" onSubmit={submitCodePlugin}>
        <div className="builder-title">
