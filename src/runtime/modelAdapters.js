export const MODEL_ADAPTERS = [
  {
    ...
    mode: '开发阶段',
    providerLabel: '本地 Qwen2.5-Omni 调试服务',
    endpoint: 'ws://localhost:8000/omni/realtime',
  ...
  },
  {
    ...
    upload: '需开启 voice.cloud_upload / camera.cloud_upload',
...
export function getNetworkLabel(mode) {
  if (mode === 'local_dev') return 'Wi‑Fi / Local Dev';
  if (mode === 'wifi_cloud') return 'Wi‑Fi / Cloud Omni';
  if (mode === 'cellular_cloud') return 'eSIM / Physical SIM Cloud';
  if (mode === 'self_hosted_cloud') return 'Wi‑Fi / Self-hosted Cloud';
  return 'Offline Pet Engine';
}
